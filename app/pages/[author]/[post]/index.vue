...
<template>
  <div>
    ...
    <div
      v-else-if="error"
      class="flex flex-col justify-center items-center min-h-48 gap-2 p-4 pb-2"
    >
      <template v-if="error.statusCode == 404">
        <Icon name="mingcute:ufo-2-line" :size="48" class="text-muted-foreground" />
        <p class="text-sm text-muted-foreground mb-2">Пост не найден</p>
      </template>
      <template v-else>
        <Icon name="mingcute:warning-line" :size="48" class="text-muted-foreground" />
        <p class="text-sm text-muted-foreground mb-2">Ошибка получения поста</p>
      </template>
      <button
